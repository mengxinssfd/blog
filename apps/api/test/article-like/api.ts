...
export const prefix = '/api/article-like';
...
export function useApi(request: () => SuperTest<Test>) {
  return {
    ...
    setLike(articleId: number, token?: string) {
      return request()
        .patch(prefix + '/' + articleId)
        .set('authorization', 'Bearer ' + token);
    },
    ...
    countByArticleId(articleId: number, token?: string) {
      return request()
        .get(prefix + '/count/' + articleId)
        .set('authorization', 'Bearer ' + token);
    },
    list(token?: string) {
      return request()
...
export const ResType = {
  ...
  created: /\{"code":200,"msg":"Success","data":\{"count":\d+,"checked":\d+}}/,
