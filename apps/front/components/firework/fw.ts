...
fscreen.addEventListener('fullscreenchange', () => {
  Store.ins.setState({ fullscreen: isFullscreen() });
});

type Listener = (
  state: typeof Store.prototype.state,
  prevState: typeof Store.prototype.state,
) => void;

// Simple state container; the source of truth.
class Store {
  private static Ins: Store;
  static get ins(): Store {
    if (!Store.Ins) Store.Ins = new Store();
    return Store.Ins;
  }
  state = {
    ...
    soundEnabled: false,
    menuOpen: false,
    openHelpTopic: null as null | (typeof nodeKeyToHelpKey)[keyof typeof nodeKeyToHelpKey],
  ...
  private _dispatch(prevState: typeof this.state) {
  ...
  // Mutates state because `Store.ins.load()` should only be called once immediately after store is created, before any subscriptions.
  load(): void {
...
function togglePause(toggle?: boolean): void {
  const paused = Store.ins.state.paused;
  let newValue;
  ...
  if (paused !== newValue) {
    Store.ins.setState({ paused: newValue });
  }
}

function toggleSound(toggle?: boolean): void {
  if (typeof toggle === 'boolean') {
    Store.ins.setState({ soundEnabled: toggle });
  } else {
    Store.ins.setState({ soundEnabled: !Store.ins.state.soundEnabled });
  }
}

function toggleMenu(toggle?: boolean): void {
  if (typeof toggle === 'boolean') {
    Store.ins.setState({ menuOpen: toggle });
  } else {
    Store.ins.setState({ menuOpen: !Store.ins.state.menuOpen });
  }
}

function updateConfig(nextConfig?: Partial<typeof Store.prototype.state.config>): void {
  nextConfig = nextConfig || getConfigFromDOM();
  Store.ins.setState({
    config: Object.assign({}, Store.ins.state.config, nextConfig),
  });
...
const isRunning = (state = Store.ins.state) => !state.paused && !state.menuOpen;
// Whether user has enabled sound.
const soundEnabledSelector = (state = Store.ins.state) => state.soundEnabled;
// Whether any sounds are allowed, taking into account multiple factors.
const canPlaySoundSelector = (state = Store.ins.state) =>
  isRunning(state) && soundEnabledSelector(state);
// Convert quality to number.
const qualitySelector = () => +Store.ins.state.config.quality;
const shellNameSelector = () => Store.ins.state.config.shell as (typeof shellNames)[number];
// Convert shell size to number.
const shellSizeSelector = () => +Store.ins.state.config.size;
const finaleSelector = () => Store.ins.state.config.finale;
const skyLightingSelector = () => +Store.ins.state.config.skyLighting;
const scaleFactorSelector = () => Store.ins.state.config.scaleFactor;
...
const appNodes = (Object.keys(_appNodes) as Array<keyof typeof _appNodes>).reduce((res, key) => {
  res[key] = document.querySelector(_appNodes[key])!;
  return res;
}, {} as Record<keyof typeof _appNodes, HTMLElement> & Record<'autoLaunch' | 'finaleMode' | 'hideControls' | 'fullscreen' | 'longExposure' | 'quality' | 'shellType' | 'shellSize' | 'scaleFactor' | 'skyLighting', HTMLInputElement>);
...
function renderApp(state: typeof Store.prototype.state) {
  ...
  appNodes.finaleModeFormOption.style.opacity = String(state.config.autoLaunch ? 1 : 0.32);
  ...
  appNodes.menuInnerWrap.style.opacity = String(state.openHelpTopic ? 0.12 : 1);
  appNodes.helpModal.classList.toggle('active', !!state.openHelpTopic);
  if (state.openHelpTopic) {
    const { header, body } = helpContent[state.openHelpTopic as keyof typeof helpContent];
    appNodes.helpModalHeader.textContent = header;
    appNodes.helpModalBody.textContent = body;
  }
}

Store.ins.subscribe(renderApp);

// Perform side effects on state changes
function handleStateChange(state: typeof Store.prototype.state, prevState: typeof state) {
...
Store.ins.subscribe(handleStateChange);
...
(Object.keys(nodeKeyToHelpKey) as Array<keyof typeof nodeKeyToHelpKey>).forEach((nodeKey) => {
  const helpKey = nodeKeyToHelpKey[nodeKey];
  appNodes[nodeKey].addEventListener('click', () => {
    Store.ins.setState({ openHelpTopic: helpKey });
  });
});

appNodes.helpModalCloseBtn.addEventListener('click', () => {
  Store.ins.setState({ openHelpTopic: null });
});
...
const COLOR_CODES: string[] = COLOR_NAMES.map((colorName) => COLOR[colorName]);
// Invisible stars need an indentifier, even through they won't be rendered - physics still apply.
const COLOR_CODES_W_INVIS: string[] = [...COLOR_CODES, INVISIBLE];
...
const strobeShell = (size = 1): ShellOptions => {
...
const palmShell = (size = 1): ShellOptions => {
...
const ringShell = (size = 1): ShellOptions => {
  const color = randomColor();
...
const crossetteShell = (size = 1): ShellOptions => {
  const color = randomColor({ limitWhite: true });
  return {
    shellSize: size,
    spreadSize: 300 + size * 100,
...
const floralShell = (size = 1): ShellOptions => ({
  shellSize: size,
...
const fallingLeavesShell = (size = 1): ShellOptions => ({
  shellSize: size,
  color: INVISIBLE,
  spreadSize: 300 + size * 120,
  starDensity: 0.12,
...
const willowShell = (size = 1): ShellOptions => ({
  shellSize: size,
  spreadSize: 300 + size * 100,
  starDensity: 0.6,
...
const crackleShell = (size = 1): ShellOptions => {
  // favor gold
...
const horsetailShell = (size = 1): ShellOptions => {
  const color = randomColor();
  return {
    shellSize: size,
    horsetail: true,
...
function randomShellName(): (typeof shellNames)[number] {
  return Math.random() < 0.5
    ? 'Crysanthemum'
    : shellNames[(Math.random() * (shellNames.length - 1) + 1) | 0];
}

function randomShell(size?: number): ShellOptions {
...
function shellFromConfig(size: number): ShellOptions {
  return shellTypes[shellNameSelector() as (typeof shellNames)[number]](size);
}
...
function randomFastShell(): (size?: number) => ShellOptions {
...
const shellNames = Object.keys(shellTypes) as Array<keyof typeof shellTypes>;
...
function seqPyramid(): number {
  ...
  function launchShell(x: number, useSpecial: boolean): void {
    const isRandom = shellNameSelector() === 'Random';
...
function handlePointerStart(event: MouseEvent) {
  activePointerCount++;
  const btnSize = 50;
...
function updateSpeedFromEvent(event: MouseEvent) {
...
function updateGlobals(timeStep: number, lag: number): void {
  ...
  // auto launch shells
  if (Store.ins.state.config.autoLaunch) {
...
function render(speed: number) {
  ...
  //   Store.ins.state.config.longExposure ? 0.0025 : 0.175 * speed
  // })`;
...
function colorSky(speed: number): void {
  ...
  appNodes.canvasContainer.style.backgroundColor = `rgba(${currentSkyColor.r | 0}, ${
    currentSkyColor.g | 0
  }, ${currentSkyColor.b | 0}, 0.5)`;
}
...
function crossetteEffect(star: Star): void {
...
function floralEffect(star: Star): void {
...
function fallingLeavesEffect(star: Star): void {
...
function crackleEffect(star: Star): void {
...
class Shell {
  ...
  spreadSize!: number;
  starCount!: number;
  glitter!: 'light' | 'medium' | 'heavy' | 'streamer' | 'willow' | 'thick';
  crackle!: number;
  ...
  constructor(options: ShellOptions) {
    ...
    this.color = color;
    this.glitterColor = options.glitterColor || (Array.isArray(color) ? color[0]! : color);
  ...
  burst(x: number, y: number) {
    ...
    let sparkFreq = 0;
    let sparkSpeed: number;
    let sparkLife: number;
    ...
    if (typeof this.color === 'string') {
      if (this.color === 'random') {
        color = undefined; // falsy value creates random color in starFactory
      } else {
        color = this.color;
      }
...
class BurstFlash {
  ...
  static _new(): BurstFlash {
    return new BurstFlash();
  }
  ...
  static add(x: number, y: number, radius: number): BurstFlash {
    const instance = BurstFlash._pool.pop() || BurstFlash._new();

    instance.x = x;
    instance.y = y;
    instance.radius = radius;

    BurstFlash.active.push(instance);
    return instance;
  }

  static returnInstance(instance: BurstFlash) {
    BurstFlash._pool.push(instance);
  }
}
...
function createParticleCollection() {
  const collection: Record<string, Star[]> = {};
  COLOR_CODES_W_INVIS.forEach((color) => {
    collection[color] = [];
  });
  return collection;
...
class Star {
  // Visual properties
  static readonly drawWidth = 3;
  static readonly airDrag = 0.98;
  static readonly airDragHeavy = 0.992;

  // Star particles will be keyed by color
  // eslint-disable-next-line no-use-before-define
  static readonly active: Record<string, Star[]> = createParticleCollection();
  // eslint-disable-next-line no-use-before-define
  private static readonly _pool: Star[] = [];

  private static _new(): Star {
    return new Star();
  }
  ...
  public sparkFreq!: number; // ms between spark emissions
  public sparkSpeed!: number;
  public sparkTimer!: number;
  public sparkColor!: string;
  public sparkLife!: number;
  public sparkLifeVariation!: number;
  public strobe!: boolean;
  public strobeFreq!: number;
  ...
  static add(
    ...
    const instance = Star._pool.pop() || Star._new();
    ...
    Star.active[color].push(instance);
    return instance;
  }

  // eslint-disable-next-line no-use-before-define
  onDeath!: null | ((ins: Star) => void);
  ...
  static returnInstance(instance: Star) {
    ...
    Star._pool.push(instance);
  }
}

class Spark {
  // Visual properties
  static drawWidth = 0; // set in `configDidUpdate()`
  static airDrag = 0.9;

  // Star particles will be keyed by color
  // eslint-disable-next-line no-use-before-define
  static active: Record<string, Spark[]> = createParticleCollection();
  // eslint-disable-next-line no-use-before-define
  static _pool: Spark[] = [];
  ...
  static add(
    ...
    const instance = Spark._pool.pop() || Spark._new();
    ...
    Spark.active[color].push(instance);
    return instance;
  }

  // Public method for cleaning up and returning an instance back to the pool.
  static returnInstance(instance: Spark): void {
    // Add back to the pool.
    Spark._pool.push(instance);
  }
}
...
const soundManager = {
  ...
  preload() {
    ...
    const types = Object.keys(this.sources) as Array<keyof typeof soundManager.sources>;
    types.forEach((type) => {
      ...
      fileNames.forEach((fileName) => {
        ...
        const promise: Promise<AudioBuffer> = fetch(fileURL)
          .then(checkStatus)
   ...
   */
  playSound(type: keyof typeof _sources, scale = 1) {
